...
bool ReadableLogSegment::DecodeEntryHeader(const Slice& data, EntryHeader* header) {
  ...
  // Verify the header.
  uint32_t computed_crc = base::Crc32c((const char *)&data[0], 8);
...
Status ReadableLogSegment::ReadEntryBatch(int64_t *offset,
  ...
  // Verify the CRC.
  uint32_t read_crc = base::Crc32c((const char *)entry_batch_slice.data(), entry_batch_slice.size());
...
Status WritableLogSegment::WriteEntryBatch(const Slice& data) {
  ...
  uint32_t msg_crc = base::Crc32c((const char *)&data[0], data.size());
  InlineEncodeFixed32(&header_buf[4], msg_crc);

  // Then the CRC of the header
  uint32_t header_crc = base::Crc32c((const char *)&header_buf, 8);
