...
#include "base/core/strings/split.h"
#include "base/core/strings/join.h"
#include "base/core/strings/numbers.h"
#include "base/core/strings/strip.h"
 ...
 private:
  ...
  void GenerateServiceIfHeader(Printer *printer,
    ...
    Print(printer, *subs,
      ...
      "\n"
      "#ifndef BASE_RPC_$upper_case$_SERVICE_IF_DOT_H\n"
      "#define BASE_RPC_$upper_case$_SERVICE_IF_DOT_H\n"
      "\n"
    ...
    for (int service_idx = 0; service_idx < file->service_count();
      ...
      Print(printer, *subs,
        "class $service_name$If : public ::base::rpc::GeneratedServiceIf {\n"
        " public:\n"
        "  explicit $service_name$If(const scoped_refptr<::base::MetricEntity>& entity,"
            " const scoped_refptr<::base::rpc::ResultTracker>& result_tracker);\n"
      ...
      for (int method_idx = 0; method_idx < service->method_count();
        ...
        "     $response$ *resp, ::base::rpc::RpcContext *context) = 0;\n"
        );
  ...
  void GenerateProxyHeader(Printer *printer,
    ...
    Print(printer, *subs,
      "// THIS FILE IS AUTOGENERATED FROM $path$\n"
      "\n"
      "#ifndef BASE_RPC_$upper_case$_PROXY_DOT_H\n"
      "#define BASE_RPC_$upper_case$_PROXY_DOT_H\n"
      "\n"
      "#include \"$path_no_extension$.pb.h\"\n"
      "\n"
    ...
    for (int service_idx = 0; service_idx < file->service_count();
      ...
      Print(printer, *subs,
        "class $service_name$Proxy : public ::base::rpc::Proxy {\n"
        " public:\n"
        "  $service_name$Proxy(const std::shared_ptr< ::base::rpc::Messenger>\n"
      ...
      for (int method_idx = 0; method_idx < service->method_count();
        ...
        Print(printer, *subs,
        "\n"
        "  ::base::Status $rpc_name$(const $request$ &req, $response$ *resp,\n"
  ...
  void GenerateProxy(Printer *printer,
    ...
    for (int service_idx = 0; service_idx < file->service_count();
      ...
      for (int method_idx = 0; method_idx < service->method_count();
        ...
        subs->PushMethod(method);
        Print(printer, *subs,
        "::base::Status $service_name$Proxy::$rpc_name$(const $request$ &req, $response$ *resp,\n"
        "                                     ::base::rpc::RpcController *controller) {\n"
        "  return SyncRequest(\"$rpc_name$\", req, resp, controller);\n"
        "}\n"
        "\n"
        "void $service_name$Proxy::$rpc_name$Async(const $request$ &req,\n"
        "                     $response$ *resp, ::base::rpc::RpcController *controller,\n"
        "                     const ::base::rpc::ResponseCallback &callback) {\n"
...
int main(int argc, char *argv[]) {
  base::rpc::CodeGenerator generator;
