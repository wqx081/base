...
#include "base/core/strings/human_readable.h"
...
class CallTransferCallbacks : public TransferCallbacks {
 public:
  explicit CallTransferCallbacks(shared_ptr<OutboundCall> call)
      : call_(std::move(call)) {}

  virtual void OnTransferFinished() OVERRIDE {
  ...
  virtual void OnTransferAborted(const Status &status) OVERRIDE {
...
Status Connection::InitSaslClient() {
  ...
  if (!gssapi_status.ok()) {
    ...
    // at that point.
    if (VLOG_IS_ON(1)) {
      LOG(INFO) << "Couldn't enable GSSAPI (Kerberos) SASL plugin: "
