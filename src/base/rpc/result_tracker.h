...
class ResultTracker : public core::RefCountedThreadSafe<ResultTracker> {
 public:
  ...
  explicit ResultTracker(std::shared_ptr<base::MemTracker> mem_tracker);
  ~ResultTracker();
 ...
 private:
  ...
  struct CompletionRecord {
    ...
    int64_t memory_footprint() const {
      return base_malloc_usable_size(this)
          + (ongoing_rpcs.capacity() > 0 ? base_malloc_usable_size(ongoing_rpcs.data()) : 0)
  ...
  struct ClientState {
    ...
    void GCCompletionRecords(const std::shared_ptr<base::MemTracker>& mem_tracker,
