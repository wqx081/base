#include "base/app/zk/storage/meta.h"
...
void Meta::WriteCurrentTerm(int64_t current_term) {
    ...
    DCHECK(fflush(term_file_) != 0)
        << strings::Substitute("Meta::WriteCurrentTerm failed, term:$0", 
			current_term);
...
void Meta::WriteVotedFor(int64_t term, const std::string& server_id) {
    ...
    DCHECK(fflush(vote_file_) != 0)
       <<    strings::Substitute("Meta::WriteVotedFor failed, "
			   "term:$0, server_id:$1",
			term, server_id);
}
...
void Meta::WriteRootInfo(const UserInfo& user) {
    ...
    DCHECK(fflush(root_file_) != 0) << "Meta::WriteUserList failed, username: " << user.username();
}
