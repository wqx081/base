...
TEST(UserManageTest, CommonUserTest) {
    ...
    // Login again
    std::string uuid2 = UserManager::CalcUuid("user1");
    ...
    EXPECT_EQ(ret, kOk);
    // Wrong password
    uuid2 = UserManager::CalcUuid("user2");
    ...
    std::string uuid3 = UserManager::CalcUuid("user3");
...
TEST(UserManageTest, SuperUserTest) {
    ...
    ret = user_manager.Login("user1", "123456", uuid1);
    EXPECT_EQ(ret, kOk);
    std::string uuid2 = UserManager::CalcUuid("user2");
    ret = user_manager.Login("user2", "123456", uuid2);
    EXPECT_EQ(ret, kOk);
