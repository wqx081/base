...
#ifndef BASE_UTIL_MEMORY_ARENA_H_
#define BASE_UTIL_MEMORY_ARENA_H_
...
#include "base/util/memory/memory.h"
...
class ArenaBase {
 ...
 private:
  ...
  inline Component* AcquireLoadCurrent() {
    if (THREADSAFE) {
      return reinterpret_cast<Component*>(
        core::subtle::Acquire_Load(reinterpret_cast<AtomicWord*>(&current_)));
    } else {
      return current_;
    }
  }
  ...
  inline void ReleaseStoreCurrent(Component* c) {
    if (THREADSAFE) {
      core::subtle::Release_Store(reinterpret_cast<AtomicWord*>(&current_),
...
inline uint8_t *ArenaBase<true>::Component::AllocateBytesAligned(
  ...
  Atomic32 aligned = MPR_ALIGN_UP(offset, alignment);
...
inline uint8_t *ArenaBase<false>::Component::AllocateBytesAligned(
  ...
  DCHECK(alignment == 1 || alignment == 2 || alignment == 4 ||
         alignment == 8 || alignment == 16)
    << "bad alignment: " << alignment;
  size_t aligned = MPR_ALIGN_UP(offset_, alignment);
...
#endif  // BASE_UTIL_MEMORY_ARENA_H_
