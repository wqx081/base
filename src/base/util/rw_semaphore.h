...
class rw_semaphore {
 public:
  ...
  void lock_shared() {
    int loop_count = 0;
    Atomic32 cur_state = core::subtle::NoBarrier_Load(&state_);
    while (true) {
      Atomic32 expected = cur_state & kNumReadersMask;   // I expect no write lock
      Atomic32 try_new_state = expected + 1;          // Add me as reader
      cur_state = core::subtle::Acquire_CompareAndSwap(&state_, expected, try_new_state);
  ...
  void unlock_shared() {
    ...
    while (true) {
      ...
      cur_state = core::subtle::Release_CompareAndSwap(&state_, expected, try_new_state);
      if (cur_state == expected)
        break;
      // Either was already locked by someone else, or CAS failed.
      yield(loop_count++);
  ...
  void lock() {
    ...
    while (true) {
      ...
      cur_state = core::subtle::NoBarrier_CompareAndSwap(&state_, expected, try_new_state);
      if (cur_state == expected)
        break;
      // Either was already locked by someone else, or CAS failed.
      yield(loop_count++);
  ...
  void unlock() {
    ...
    DCHECK_EQ(core::subtle::NoBarrier_Load(&state_), kWriteFlag);
  ...
  bool is_write_locked() const {
    return core::subtle::NoBarrier_Load(&state_) & kWriteFlag;
  }
  ...
  bool is_locked() const {
    return core::subtle::NoBarrier_Load(&state_);
  }
 ...
 private:
  ...
  void WaitPendingReaders() {
    int loop_count = 0;
    while ((core::subtle::Acquire_Load(&state_) & kNumReadersMask) > 0) {
      yield(loop_count++);
    }
  }
