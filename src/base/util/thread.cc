...
//#include "base/util/kernel_stack_watchdog.h"
#include "base/util/metrics.h"
#include "base/util/mutex.h"
#include "base/util/os_util.h"
...
METRIC_DEFINE_gauge_uint64(server, voluntary_context_switches,
                           ...
                           base::MetricUnit::kContextSwitches,
...
void ThreadMgr::PrintThreadCategoryRows(const ThreadCategory& category,
  ...
  for (const ThreadCategory::value_type& thread : category) {
    ...
    if (!status.ok()) {
      LOG(INFO) << "Could not get per-thread statistics: "
                              << status.ToString();
    }
...
void* Thread::SuperviseThread(void* arg) {
  ...
  if (system_tid == -1) {
    ...
    LOG(INFO) << "Could not determine thread ID: " << error_msg;
  }
