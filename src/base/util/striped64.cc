...
using base::striped64::internal::HashCode;
using base::striped64::internal::Cell;
...
void Striped64::RetryUpdate(int64_t x, Rehash contention) {
  ...
  // infinite retry loop.
  while (true) {
    int32_t n = core::subtle::Acquire_Load(&num_cells_);
    if (n > 0) {
      ...
      n = core::subtle::Acquire_Load(&num_cells_);
      if (n == 0) {
        n = 1;
        ...
        core::subtle::Release_Store(&num_cells_, n);
      }
...
void Striped64::InternalReset(int64_t initialValue) {
  const int32_t n = core::subtle::Acquire_Load(&num_cells_);
