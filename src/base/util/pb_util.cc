...
#include "base/util/coding-inl.h"
#include "base/util/coding.h"
#include "base/util/crc32c.h"
#include "base/util/env.h"
#include "base/util/env_util.h"
#include "base/util/jsonwriter.h"
#include "base/util/mutex.h"
#include "base/util/path_util.h"
#include "base/util/pb_util-internal.h"
...
using base::pb_util::internal::SequentialFileFileInputStream;
using base::pb_util::internal::WritableFileOutputStream;
using std::deque;
using std::endl;
...
Status ParseAndCompareChecksum(const uint8_t* checksum_buf,
  ...
  for (Slice s : slices) {
    ...
    actual_checksum = base::Crc32c((const char *)s.data(), s.size());
  }
...
Status WritablePBContainerFile::Init(const Message& msg) {
  ...
  if (version_ >= 2) {
    uint32_t header_checksum = Crc32c((const char *)buf.data(), offset);
...
Status WritablePBContainerFile::AppendMsgToBuffer(const Message& msg, faststring* buf) {
  ...
  if (version_ >= 2) {
    uint32_t length_checksum = Crc32c((const char *)&data_len, sizeof(data_len));
  ...
  if (version_ == 1) {
    data_checksum = Crc32c((const char *)dst, cur_offset);
  } else {
    data_checksum = Crc32c((const char *)(dst + data_offset), data_len);
  }
