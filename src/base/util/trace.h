#ifndef BASE_UTIL_TRACE_H
#define BASE_UTIL_TRACE_H
...
#define ADOPT_TRACE(t) base::ScopedAdoptTrace _adopt_trace(t);
...
#define TRACE(format, substitutions...) \
  do { \
    base::Trace* _trace = Trace::CurrentTrace(); \
    if (_trace) { \
...
#define TRACE_COUNTER_SCOPE_LATENCY_US(counter_name) \
  ::base::ScopedTraceLatencyCounter _scoped_latency(counter_name)
...
class Trace : public core::RefCountedThreadSafe<Trace> {
 public:
  Trace();
  ...
  // 'call base::Trace::DumpCurrentTrace();'.
