...
Status DecodeThreeChars(const char* codes, char* result) {
  ...
  if (PREDICT_FALSE((packed & 0xFF000000) != 0)) {
    return Status::InvalidArgument("Invalid character found in base64.");
  }
...
Status Base64Decode(StringPiece data, std::string* decoded) {
  if (decoded == nullptr) {
    //return Status::Internal("'decoded' cannot be nullptr.");
    CHECK(false);
  }
  ...
  while (end - b64 > 4) {
    RETURN_NOT_OK(DecodeThreeChars(b64, current));
    b64 += 4;
    current += 3;
  }
  ...
  RETURN_NOT_OK(DecodeThreeChars(tail, current));
...
Status Base64Encode(StringPiece source, std::string* encoded) {
  return Base64Encode(source, false, encoded);
}

Status Base64Encode(StringPiece source, bool with_padding, std::string* encoded) {
