#ifndef BASE_UTIL_METRICS_H
#define BASE_UTIL_METRICS_H
...
//                            "Threads Started",
//                            base::MetricUnit::kThreads,
//                            "Total number of threads started on this server",
//                            base::EXPOSE_AS_COUNTER);
//
//
// Metrics ownership
// ------------------------------------------------------------
...
//
//   METRIC_DEFINE_counter(server, ping_requests, "Ping Requests", base::MetricUnit::kRequests,
...
//   METRIC_DEFINE_counter(my_entity, ping_requests, "Ping Requests", base::MetricUnit::kRequests,
...
#define METRIC_DEFINE_entity(name)                               \
  ::base::MetricEntityPrototype METRIC_ENTITY_##name(#name)
...
#define METRIC_DECLARE_entity(name) \
  extern ::base::MetricEntityPrototype METRIC_ENTITY_##name
#define METRIC_DECLARE_counter(name)                             \
  extern ::base::CounterPrototype METRIC_##name
#define METRIC_DECLARE_gauge_string(name) \
  extern ::base::GaugePrototype<std::string> METRIC_##name
#define METRIC_DECLARE_gauge_bool(name) \
  extern ::base::GaugePrototype<bool> METRIC_##name
#define METRIC_DECLARE_gauge_int32(name) \
  extern ::base::GaugePrototype<int32_t> METRIC_##name
#define METRIC_DECLARE_gauge_uint32(name) \
  extern ::base::GaugePrototype<uint32_t> METRIC_##name
#define METRIC_DECLARE_gauge_int64(name) \
  extern ::base::GaugePrototype<int64_t> METRIC_##name
#define METRIC_DECLARE_gauge_uint64(name) \
  extern ::base::GaugePrototype<uint64_t> METRIC_##name
#define METRIC_DECLARE_gauge_double(name) \
  extern ::base::GaugePrototype<double> METRIC_##name
#define METRIC_DECLARE_histogram(name) \
  extern ::base::HistogramPrototype METRIC_##name

#if defined(__APPLE__)
#define METRIC_DEFINE_gauge_size(entity, name, label, unit, desc, ...) \
  ::base::GaugePrototype<size_t> METRIC_##name(                    \
      ::base::MetricPrototype::CtorArgs(#entity, #name, label, unit, desc, ## __VA_ARGS__))
#define METRIC_DECLARE_gauge_size(name) \
  extern ::base::GaugePrototype<size_t> METRIC_##name
#else
...
class MetricEntity : public core::RefCountedThreadSafe<MetricEntity> {
 public:
  ...
  scoped_refptr<FunctionGauge<T> > FindOrCreateFunctionGauge(const GaugePrototype<T>* proto,
                                                             const core::Callback<T()>& function);
...
class Metric : public core::RefCountedThreadSafe<Metric> {
 public:
...
class GaugePrototype : public MetricPrototype {
 public:
  ...
  scoped_refptr<FunctionGauge<T> > InstantiateFunctionGauge(
      const scoped_refptr<MetricEntity>& entity,
      const core::Callback<T()>& function) const {
...
class FunctionGaugeDetacher {
 ...
 private:
  ...
  void OnDestructor(const core::Closure& c) {
    callbacks_.push_back(c);
  }

  std::vector<core::Closure> callbacks_;
...
class FunctionGauge : public Gauge {
 ...
 private:
  ...
  FunctionGauge(const GaugePrototype<T>* proto, core::Callback<T()> function)
