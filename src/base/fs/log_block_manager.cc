#include "base/fs/log_block_manager.h"
...
#include "base/fs/block_manager_util.h"
#include "base/fs/data_dirs.h"
#include "base/fs/fs.pb.h"
#include "base/fs/file_cache.h"
...
METRIC_DEFINE_counter(server, log_block_manager_containers,
                      ...
                      base::MetricUnit::kLogBlockContainers,
...
Status LogBlockContainer::Open(LogBlockManager* block_manager,
  ...
  // safely delete.
  {
    ...
    if (metadata_size < pb_util::kPBContainerMinimumValidLength &&
      ...
      if (!ro) {
core::IgnoreResult(env->DeleteFile(metadata_path));
core::IgnoreResult(env->DeleteFile(data_path));
      }
...
Status LogBlockContainer::DeleteBlock(int64_t offset, int64_t length) {
  ...
  if (length) {
    ...
    // our request.
    return data_file_->PunchHole(offset, MPR_ALIGN_UP(
...
void LogBlockContainer::UpdateBytesWrittenAndTotalBlocks(int64_t block_offset,
  ...
  int64_t new_total_bytes = MPR_ALIGN_UP(
...
class LogBlock : public core::RefCountedThreadSafe<LogBlock> {
 public:
...
size_t LogReadableBlock::memory_footprint() const {
  return base_malloc_usable_size(this);
}
...
LogBlockManager::~LogBlockManager() {
  ...
  for (const auto& entry : blocks_by_block_id_) {
    mem += base_malloc_usable_size(entry.second.get());
  }
...
bool LogBlockManager::AddLogBlock(LogBlockContainer* container,
  ...
  mem_tracker_->Consume(base_malloc_usable_size(lb.get()));
...
scoped_refptr<LogBlock> LogBlockManager::RemoveLogBlock(const BlockId& block_id) {
  ...
  if (result) {
    ...
    mem_tracker_->Release(base_malloc_usable_size(result.get()));
  ...
  for (const string& child : children) {
    ...
    // the container.
    {
      ...
      for (const UntrackedBlockMap::value_type& e : blocks_in_container) {
        ...
        mem_usage += base_malloc_usable_size(e.second.get());
      }
