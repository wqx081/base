...
#include "base/fs/cache.h"
#include "base/fs/cache_metrics.h"
#include "base/fs/nvm_cache.h"
...
struct LRUHandle {
  ...
  uint8_t* mutable_val_ptr() {
    int val_offset = MPR_ALIGN_UP(key_length, sizeof(void*));
...
bool LRUCache::Unref(LRUHandle* e) {
  ...
  return !core::RefCountDec(&e->refs);
}
...
int DetermineShardBits() {
  int bits = PREDICT_FALSE(FLAGS_cache_force_single_shard) ?
      0 : Bits::Log2Ceiling(core::NumCPUs());
...
class ShardedLRUCache : public Cache {
 ...
 public:
  ...
  virtual PendingHandle* Allocate(Slice key, int val_len, int charge) OVERRIDE {
    ...
    DCHECK_GE(key_len, 0);
    DCHECK_GE(val_len, 0);
    int key_len_padded = MPR_ALIGN_UP(key_len, sizeof(void*));
