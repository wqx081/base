...
#include "base/rpc/sasl_client.h"
#include "base/rpc/sasl_common.h"
#include "base/rpc/sasl_server.h"
#include "base/security/test/mini_kdc.h"
#include "base/util/monotime.h"
#include "base/util/net/sockaddr.h"
#include "base/util/net/socket.h"
...
TEST_F(TestSaslRpc, TestRestrictiveServer_NonRestrictiveClient) {
  ...
  string kt_path;
  ASSERT_OK(kdc.CreateServiceKeytab("base/127.0.0.1", &kt_path));
...
TEST_F(TestSaslRpc, TestNoMatchingMechanisms) {
  ...
  // Create the server principal and keytab.
  string kt_path;
  ASSERT_OK(kdc.CreateServiceKeytab("base/localhost", &kt_path));
  CHECK_ERR(setenv("KRB5_KTNAME", kt_path.c_str(), 1 /*replace*/));
      ...
      std::bind(RunGSSAPINegotiationServer, std::placeholders::_1,
                [](const Status& s, SaslServer& server) {
                  ...
                  ASSERT_STR_CONTAINS(s.ToString(),
                                      "No key table entry found matching base/127.0.0.1");
#endif
                }),
      std::bind(RunGSSAPINegotiationClient, std::placeholders::_1,
                [](const Status& s, SaslClient& client) {
...
TEST_F(TestSaslRpc, TestPreflight) {
  ...
  ASSERT_FALSE(s.ok());
#ifndef KRB5_VERSION_LE_1_10
  ASSERT_STR_MATCHES(s.ToString(), "No key table entry found matching base/.*");
#endif
}
#endif
