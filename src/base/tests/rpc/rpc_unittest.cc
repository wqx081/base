...
METRIC_DECLARE_histogram(handler_latency_base_rpc_test_CalculatorService_Sleep);
...
TEST_P(TestRpc, TestCall) {
  ...
  ASSERT_STR_CONTAINS(p.ToString(), strings::Substitute("base.rpc.GenericCalculatorService@"
...
TEST_P(TestRpc, TestRpcHandlerLatencyMetric) {
  ...
  scoped_refptr<Histogram> latency_histogram = down_cast<Histogram *>(
      FindOrDie(metric_map,
                &METRIC_handler_latency_base_rpc_test_CalculatorService_Sleep).get());
