...
#include "base/util/subprocess.h"
...
TEST_F(RpcStubTest, TestCallWithInvalidParam) {
  ...
  ASSERT_STR_CONTAINS(s.ToString(),
                      ...
                      "base.rpc_test.CalculatorService.Add: "
                      "missing fields: y");
}
...
static void DoIncrement(Atomic32* count) {
  core::subtle::Barrier_AtomicIncrement(count, 1);
}
...
TEST_F(RpcStubTest, TestCallWithMissingPBFieldClientSide) {
  ...
  ASSERT_STR_CONTAINS(controller.status().ToString(),
                      "Invalid argument: invalid parameter for call "
                      "base.rpc_test.CalculatorService.Add: missing fields: y");
}
...
TEST_F(RpcStubTest, TestApplicationError) {
  ...
  EXPECT_EQ("message: \"Got some error\"\n"
            "[base.rpc_test.CalculatorError.app_error_ext] {\n"
...
namespace {
struct RefCountedTest : public core::RefCountedThreadSafe<RefCountedTest> {
};
