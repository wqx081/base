...
struct GoogleOnceType {
  core::subtle::Atomic32 state;
};

#define GOOGLE_ONCE_INIT { GOOGLE_ONCE_INTERNAL_INIT }

// For internal use only.
extern void GoogleOnceInternalInit(core::subtle::Atomic32* state, void (*func)(),
                                   void (*func_with_arg)(void*), void* arg);

inline void GoogleOnceInit(GoogleOnceType* state, void (*func)()) {
core::subtle::Atomic32 s = core::subtle::Acquire_Load(&state->state);
...
core::subtle::Atomic32 s = core::subtle::Acquire_Load(&state->state);
  if (PREDICT_FALSE(s != GOOGLE_ONCE_INTERNAL_DONE)) {
    ...
    typedef typename core::remove_const<T>::type mutable_T;
...
core::subtle::Atomic32 s = core::subtle::Acquire_Load(&this->state_);
    if (PREDICT_FALSE(s != GOOGLE_ONCE_INTERNAL_DONE)) {
      // Deal with const T as well as non-const T.
      typedef typename core::remove_const<T>::type mutable_T;
