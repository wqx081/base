...
#pragma GCC diagnostic pop
 ...
 private:
  ...
  template <typename T>
  struct is_map {
    template <typename U> static core::big_ test(typename U::mapped_type*);
    template <typename> static core::small_ test(...);
    static const bool value = (sizeof(test<T>(0)) == sizeof(core::big_));
  };
