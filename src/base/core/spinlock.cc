...
#include "base/core/synchronization_profiling.h"
#include "base/core/spinlock_internal.h"
#include "base/core/walltime.h"
#include "base/core/sysinfo.h"   /* for NumCPUs() */
...
const core::LinkerInitialized SpinLock::LINKER_INITIALIZED =
    core::LINKER_INITIALIZED;
...
core::subtle::Atomic32 SpinLock::SpinLoop(int64 initial_wait_timestamp,
                            core::subtle::Atomic32* wait_cycles) {
  int c = adaptive_spin_count;
  while (core::subtle::NoBarrier_Load(&lockword_) != kSpinLockFree && --c > 0) {
    core::subtle::PauseCPU();
  }
core::subtle::Atomic32 spin_loop_wait_cycles = CalculateWaitCycles(initial_wait_timestamp);
core::subtle::Atomic32 lock_value =
      core::subtle::Acquire_CompareAndSwap(&lockword_, kSpinLockFree,
...
core::subtle::Atomic32 wait_cycles;
core::subtle::Atomic32 lock_value = SpinLoop(wait_start_time, &wait_cycles);

  int lock_wait_call_count = 0;
  lock_wait_call_count = 0;
  ...
  while (lock_value != kSpinLockFree) {
    ...
    if (lock_value == kSpinLockHeld) {
      ...
      lock_value = core::subtle::Acquire_CompareAndSwap(&lockword_,
                                                        kSpinLockHeld,
                                                        kSpinLockSleeper);
      if (lock_value == kSpinLockHeld) {
    ...
    core::internal::SpinLockDelay(&lockword_, lock_value,
...
void SpinLock::SlowUnlock(uint64 wait_cycles) {
  core::internal::SpinLockWake(&lockword_, false);  // wake waiter if necessary
  ...
  if (wait_cycles > kSpinLockSleeper) {
    SubmitSpinLockProfileData(this,
