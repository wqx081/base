...
//
// core::identity_ is used to make a non-deduced context, which
// forces all callers to explicitly specify the template argument.
template<typename To>
inline To implicit_cast(typename core::identity_<To>::type to) {
  return to;
}
...
inline To implicit_cast(typename core::identity_<From>::type const &f) {
  return f;
}
...
inline To down_cast(From& f) {
  COMPILE_ASSERT(core::is_reference<To>::value, target_type_not_a_reference);
  typedef typename core::remove_reference<To>::type* ToAsPointer;
  if (false) {
...
inline Enum loose_enum_cast(int e_val) {
  if (!loose_enum_test<Enum>(e_val)) {
    core::internal::WarnEnumCastError(e_val);
  }
